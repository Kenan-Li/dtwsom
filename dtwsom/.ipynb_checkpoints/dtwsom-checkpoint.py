...
class DtwSom(MiniSom):
    def __init__(self, x, y, input_len, sigma=1.0, learning_rate=0.5,
        ...
        super().__init__(self, x, y, input_len, sigma=sigma, learning_rate=learning_rate,
                         decay_function=decay_function,
                         neighborhood_function=neighborhood_function, random_seed=random_seed)
        self.gl_const = gl_const
        self.scr = scr
        self.ims = ims
    def DTW_update(self, s1, s2, w, gl_const=self.gl_const, scr=self.scr, ims=self.ims):
        """find the best path"""
        best_path, distance = dtw_path(s1, s2, global_constraint=gl_const, sakoe_chiba_radius=scr,itakura_max_slope=ims)
        x_cords = []
        y_cords = []
        # print(w)
        for i in best_path:
    ...
    def _activate(self, x, gl_const=self.gl_const, scr=self.scr, ims=self.ims):
        """Updates matrix activation_map, in this matrix
           the element i,j is the response of the neuron i,j to x."""
        # s = subtract(x, self._weights)  # x - w
        it = nditer(self._activation_map, flags=['multi_index'])
        while not it.finished:
            ...
            self._activation_map[it.multi_index] = dtw(x, self._weights[it.multi_index],
                                                      global_constraint=gl_const, sakoe_chiba_radius=scr,itakura_max_slope=ims)
            it.iternext()
    def activate(self, x, gl_const=self.gl_const, scr=self.scr, ims=self.ims):
        """Returns the activation map to x."""
        self._activate(x, gl_const=gl_const, scr=scr, ims=ims)
        return self._activation_map

    def update(self, x, win, t, max_iteration, gl_const=self.gl_const, scr=self.scr, ims=self.ims):
        ...
        while not it.finished:
            ...
            self._weights[it.multi_index] = self.DTW_update(x, self._weights[it.multi_index], g[it.multi_index],
                                                            gl_const=gl_const, scr=scr, ims=ims)
            it.iternext()

    def distance_map(self, gl_const=self.gl_const, scr=self.scr, ims=self.ims):
        """Returns the distance map of the weights.
        Each cell is the normalised sum of the distances between
        a neuron and its neighbours."""
        um = zeros((self._weights.shape[0], self._weights.shape[1]))
            ...
            for ii in range(it.multi_index[0] - 1, it.multi_index[0] + 2):
                for jj in range(it.multi_index[1] - 1, it.multi_index[1] + 2):
                    if (ii >= 0 and ii < self._weights.shape[0] and
                        ...
                        w_1 = self._weights[ii, jj, :]
                        w_2 = self._weights[it.multi_index]
                        um[it.multi_index] += dtw(w_1, w_2,global_constraint=gl_const, sakoe_chiba_radius=scr,itakura_max_slope=ims)
            it.iternext()
        um = um / um.max()
        return um

    def quantization_error(self, data, gl_const=self.gl_const, scr=self.scr, ims=self.ims):
        """Returns the quantization error computed as the average
        distance between each input sample and its best matching unit."""
        self._check_input_len(data)
        error = 0
        for x in data:
            error += dtw(x, self._weights[self.winner(x)],global_constraint=gl_const, sakoe_chiba_radius=scr,itakura_max_slope=ims)
        return error / len(data)
    def train_random(self, data, num_iteration, verbose=False, gl_const=self.gl_const, scr=self.scr, ims=self.ims):
        ...
        for iteration in iterations:
            ...
            rand_i = self._random_generator.randint(len(data))
            self.update(data[rand_i], self.winner(data[rand_i]),
                        iteration, num_iteration, gl_const=gl_const, scr=scr, ims=ims)
        if verbose:
            print(' - quantization error:', self.quantization_error(data))

    def train_batch(self, data, num_iteration, verbose=False, gl_const=self.gl_const, scr=self.scr, ims=self.ims):
